...
const MasterPemasok = () => {
  ...
  const handleDelete = async (pemasok) => {
    if (!window.confirm(`Apakah Anda yakin ingin menghapus pemasok "${pemasok.nama}" (${pemasok.id})?`)) {
      return;
    }

    try {
      await apiFetch(`/pemasok/${pemasok.id}`, {
        method: 'DELETE'
      });
      showMessage('Pemasok berhasil dihapus', 'success');
      loadPemasok();
    } catch (error) {
      console.error("Gagal hapus pemasok:", error);
    }
  };
